...
class HierarchicalKTModel(nn.Module):
    ...
    def forward(
        ...
        # Embed inputs
        exercise_emb = self.exercise_embedding(
            exercise_ids)  # (batch, seq_len, emb_dim)
        compilation_emb = self.compilation_embedding(
            compilation_labels)  # (batch, seq_len, emb_dim//2)

        # Handle execution labels: replace -1 with padding index
        execution_labels_valid = execution_labels.clone()
        execution_labels_valid[execution_labels == -
                               1] = self.num_execution_classes
        execution_emb = self.execution_embedding(
            execution_labels_valid)  # (batch, seq_len, emb_dim//2)

        # Concatenate embeddings
        combined_emb = torch.cat(
            [exercise_emb, compilation_emb, execution_emb], dim=-1)
        ...
        # (batch, seq_len, num_ex * num_comp_classes)
        compilation_logits = self.compilation_head(lstm_out)
        ...
        # (batch, seq_len, num_ex, num_comp_classes)
        compilation_probs = F.softmax(compilation_logits, dim=-1)
        ...
        # (batch, seq_len, num_comp_classes)
        compilation_probs_agg, _ = torch.max(compilation_probs, dim=2)
        ...
        # (batch, seq_len, num_ex * num_exec_classes)
        execution_logits = self.execution_head(execution_input)
    ...
    def predict(
        ...
        outputs = self.forward(
            exercise_ids, compilation_labels, execution_labels, seq_lens)
        ...
        # (batch, seq_len, num_ex, 1)
        target_mask = target_exercises_one_hot.unsqueeze(-1)

        # (batch, seq_len, num_comp)
        compilation_logits_target = (
            compilation_logits * target_mask).sum(dim=2)
        # (batch, seq_len, num_exec)
        execution_logits_target = (execution_logits * target_mask).sum(dim=2)
    ...
    def get_hierarchical_prediction(
        ...
        prob_execution_outcomes = execution_probs * \
            prob_compiled  # (batch, seq_len, 7)

        # Combine: [P(CE), P(AC|compiled)*P(compiled), P(WA|compiled)*P(compiled), ...]
        combined_probs = torch.cat(
            [prob_ce, prob_execution_outcomes], dim=-1)  # (batch, seq_len, 8)
...
class HierarchicalLoss(nn.Module):
    ...
    def forward(
        ...
        """
        # Flatten tensors
        compilation_logits_flat = compilation_logits.view(
            -1, compilation_logits.size(-1))
        execution_logits_flat = execution_logits.view(
            -1, execution_logits.size(-1))
